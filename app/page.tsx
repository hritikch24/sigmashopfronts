...
export default function HomePage() {
  return (
    <>
      ...
      {/* ── Hero ──────────────────────────────────────────────────────────── */}
      <section className="relative min-h-[85vh] lg:min-h-[100vh] flex items-center justify-center overflow-hidden">
        <Image
      ...
      {/* ── Services Overview ─────────────────────────────────────────────── */}
      <section className="section-padding bg-space relative overflow-hidden bg-orb-cyan" id="services" aria-labelledby="services-heading">
        <div className="absolute inset-0 bg-grid-dense pointer-events-none" />
        <div className="container-max relative z-10">
          <div className="text-center mb-12 lg:mb-16">
            <p className="text-cyan font-semibold text-sm tracking-[0.2em] uppercase mb-3">What We Do</p>
            <h2 id="services-heading" className="services-section font-heading text-3xl sm:text-4xl lg:text-5xl font-bold text-white mb-4">
              Our Commercial Shopfront &amp; Security Services
            </h2>
            <p className="text-grey-500 text-lg max-w-2xl mx-auto leading-relaxed">
      ...
      {/* ── Why Choose Sigma ─────────────────────────────────────────────── */}
      <section className="section-padding bg-space-light relative" aria-labelledby="why-heading">
        <div className="container-max">
          <div className="text-center mb-12 lg:mb-16">
            <p className="text-violet font-semibold text-sm tracking-[0.2em] uppercase mb-3">Why Sigma</p>
            <h2 id="why-heading" className="font-heading text-3xl sm:text-4xl lg:text-5xl font-bold text-white mb-4">
              Why Choose Sigma for Aluminium Shopfront Installation
            </h2>
            <p className="text-grey-500 text-lg max-w-xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {usps.map((usp) => (
              <div key={usp.title} className="flex flex-col items-center text-center p-8 card-surface hover-glow transition-all duration-500">
                <div className="w-16 h-16 rounded-2xl bg-cyan/5 border border-cyan/10 flex items-center justify-center mb-5 text-cyan">
                  {usp.icon}
                </div>
                <h3 className="font-heading text-lg font-bold text-white mb-3">{usp.title}</h3>
                <p className="text-grey-500 text-sm leading-relaxed">{usp.description}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ── Trust Credentials ──────────────────────────────────────────── */}
      <section className="py-12 relative overflow-hidden border-y border-white/[0.04]" style={{ background: 'linear-gradient(90deg, rgba(0,229,255,0.02) 0%, rgba(153,69,255,0.02) 50%, rgba(0,229,255,0.02) 100%)' }}>
        <div className="container-max">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
            {[
              ...
              <div key={stat.label} className="group">
                <p className="font-heading text-3xl sm:text-4xl font-extrabold text-cyan group-hover:text-glow-cyan transition-all">{stat.value}</p>
                <p className="text-grey-400 text-sm mt-1">{stat.label}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="section-padding bg-space relative bg-orb-violet" aria-labelledby="gallery-heading">
        <div className="container-max relative z-10">
          <div className="text-center mb-12 lg:mb-16">
            <p className="text-cyan font-semibold text-sm tracking-[0.2em] uppercase mb-3">Portfolio</p>
            <h2 id="gallery-heading" className="font-heading text-3xl sm:text-4xl lg:text-5xl font-bold text-white mb-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
            {galleryImages.map(({ src, alt }, i) => (
              <Link
                key={src}
                href="/gallery"
                className={`group relative overflow-hidden rounded-xl bg-space-mid border border-white/[0.04] hover:border-cyan/20 transition-all duration-500 ${
              ...
              >
                <div className={i === 0 ? 'aspect-square md:aspect-[4/3] lg:aspect-[4/3]' : 'aspect-square'}>
                  ...
                  />
                  <div className="absolute inset-0 bg-cyan/0 group-hover:bg-cyan/10 transition-colors duration-500 flex items-center justify-center">
                    <span className="opacity-0 group-hover:opacity-100 transition-opacity duration-300 text-space text-sm font-semibold bg-cyan/80 backdrop-blur-sm px-3 py-1.5 rounded-full">
                      View Gallery
                    </span>
                  </div>
                </div>
      ...
      {/* ── Testimonials ─────────────────────────────────────────────────── */}
      <section className="section-padding bg-space-light relative" aria-labelledby="testimonials-heading">
        <div className="absolute inset-0 bg-mesh-grid pointer-events-none opacity-50" />
        <div className="container-max relative z-10">
          <div className="text-center mb-12 lg:mb-16">
            <h2 id="testimonials-heading" className="font-heading text-3xl sm:text-4xl lg:text-5xl font-bold text-white mb-4">
          ...
          <div className="flex items-center justify-center gap-3 mt-10">
            <div className="flex items-center gap-0.5" aria-label="5 out of 5 stars average">
              {Array.from({ length: 5 }).map((_, i) => (
                <svg key={i} width="20" height="20" viewBox="0 0 24 24" fill="#00e5ff" aria-hidden="true">
                  <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
                </svg>
              ))}
            </div>
            <span className="text-grey-500 text-sm font-medium">5.0 average from verified reviews</span>
          </div>
        </div>
      </section>

      {/* ── Service Areas ─────────────────────────────────────────────────── */}
      <section className="section-padding bg-space relative" aria-labelledby="areas-heading">
        <div className="container-max">
          <div className="text-center mb-12 lg:mb-16">
            <p className="text-violet font-semibold text-sm tracking-[0.2em] uppercase mb-3">Coverage</p>
            <h2 id="areas-heading" className="font-heading text-3xl sm:text-4xl lg:text-5xl font-bold text-white mb-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 mb-10">
            {serviceAreas.map(({ city, slug }) => (
              <Link
                key={slug}
                href={`/areas/${slug}`}
                className="group flex items-center gap-2 p-4 rounded-xl border border-white/[0.06] hover:border-cyan/30 hover:bg-cyan/5 transition-all duration-300"
              >
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" aria-hidden="true" className="text-cyan flex-shrink-0">
                  <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z" fill="currentColor" />
                </svg>
                <span className="text-sm font-semibold text-grey-600 group-hover:text-cyan transition-colors">{city}</span>
              </Link>
            ))}
          </div>
          ...
          <div className="flex justify-center">
            <div className="inline-flex items-center gap-3 bg-cyan/10 border border-cyan/20 text-cyan px-6 py-3 rounded-full text-sm font-semibold backdrop-blur-sm">
      ...
      <section className="section-padding bg-space-light relative" aria-labelledby="verification-heading">
        <div className="absolute inset-0 bg-grid-dense pointer-events-none opacity-50" />
        <div className="container-max relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            <div className="order-2 lg:order-1">
              <p className="text-cyan font-semibold text-sm tracking-[0.2em] uppercase mb-3">Verified Business</p>
              <h2 id="verification-heading" className="font-heading text-3xl sm:text-4xl font-bold text-white mb-5">
              ...
              <div className="card-surface p-6 rounded-2xl mb-6">
                <dl className="space-y-3">
                  <div className="flex items-start gap-3">
                    <dt className="text-grey-400 text-sm w-36 flex-shrink-0">Company Name</dt>
                    <dd className="text-white font-semibold text-sm">Sigma Shopfronts and Shutter Limited</dd>
                  </div>
                  <div className="flex items-start gap-3">
                    <dt className="text-grey-400 text-sm w-36 flex-shrink-0">Company Number</dt>
                    <dd className="text-cyan font-semibold text-sm font-mono">16794487</dd>
                  </div>
                  <div className="flex items-start gap-3">
                    <dt className="text-grey-400 text-sm w-36 flex-shrink-0">Status</dt>
                    <dd>
                      <span className="inline-flex items-center gap-1.5 bg-cyan/10 text-cyan text-xs font-semibold px-3 py-1 rounded-full border border-cyan/20">
                  ...
                  <div className="flex items-start gap-3">
                    <dt className="text-grey-400 text-sm w-36 flex-shrink-0">Registered In</dt>
                    <dd className="text-white font-semibold text-sm">England &amp; Wales</dd>
                  </div>
                </dl>
              </div>

              <div className="flex items-center gap-3 p-4 bg-cyan/5 border border-cyan/20 rounded-xl">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" aria-hidden="true" className="text-cyan flex-shrink-0">
            ...
            <div className="order-1 lg:order-2 relative">
              ...
              {/* Floating badge */}
              <div className="absolute -bottom-4 -right-4 card-surface px-5 py-4 flex items-center gap-3 border border-cyan/20 shadow-[0_0_20px_rgba(0,229,255,0.1)]">
                <svg width="28" height="28" viewBox="0 0 24 24" fill="none" aria-hidden="true" className="text-cyan">
                  <path d="M12 1L3 5v6c0 5.55 3.84 10.74 9 12 5.16-1.26 9-6.45 9-12V5l-9-4zm-2 16l-4-4 1.41-1.41L10 14.17l6.59-6.59L18 9l-8 8z" fill="currentColor" />
                </svg>
                <div>
                  <p className="text-white text-sm font-bold leading-tight">Verified Company</p>
                  <p className="text-cyan text-xs">No. 16794487</p>
                </div>
              </div>
            </div>
          </div>
      ...
      {/* ── CTA / Contact Form ───────────────────────────────────────────── */}
      <section className="section-padding bg-space relative overflow-hidden bg-orb-cyan" id="quote" aria-labelledby="cta-heading">
        {/* Decorative radial */}
        <div className="absolute -top-32 -right-32 w-96 h-96 rounded-full bg-violet/5 blur-3xl pointer-events-none" aria-hidden="true" />
        <div className="absolute -bottom-32 -left-32 w-96 h-96 rounded-full bg-cyan/5 blur-3xl pointer-events-none" aria-hidden="true" />
        <div className="absolute inset-0 bg-mesh-grid pointer-events-none opacity-30" />

        <div className="container-max relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-start">
            {/* Copy */}
            <div className="lg:pt-4">
              <p className="text-cyan font-semibold text-sm tracking-widest uppercase mb-3">Get Started</p>
              ...
              <ul className="space-y-4 mb-10">
                {[
                  ...
                  <li key={item} className="flex items-center gap-3 text-grey-600 text-base">
              ...
              <div className="card-surface p-5">
                ...
                <a
                  href="tel:07414779594"
                  className="flex items-center gap-2 text-cyan text-xl font-bold hover:text-cyan-light transition-colors"
                >
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                    <path d="M6.62 10.79c1.44 2.83 3.76 5.14 6.59 6.59l2.2-2.2c.27-.27.67-.36 1.02-.24 1.12.37 2.33.57 3.57.57.55 0 1 .45 1 1V20c0 .55-.45 1-1 1-9.39 0-17-7.61-17-17 0-.55.45-1 1-1h3.5c.55 0 1 .45 1 1 0 1.25.2 2.45.57 3.57.11.35.03.74-.25 1.02l-2.2 2.2z" />
                  </svg>
